...
import { archiveItemIdSchema, archiveItemByIdSchema, bulkArchiveActionSchema, createArchiveItemSchema, listArchiveItemsSchema, updateArchiveItemSchema } from './archive-items.schemas.js';
...
const includeConfig = {
    ...
    files: { orderBy: [{ isPrimary: 'desc' }, { sortOrder: 'asc' }, { createdAt: 'desc' }] }
};
const listSelect = {
    id: true,
    slug: true,
...
const canAccessItem = (roles, accessLevel) => {
    if (accessLevel === 'PUBLIC')
        return true;
    if (accessLevel === 'STAFF_ONLY')
        return roles?.includes('STAFF') || roles?.includes('ADMIN') || false;
    return roles?.includes('ADMIN') || false;
};
const ensureCanManageItem = async (itemId, user) => {
...
archiveItemsRouter.get('/', optionalAuth, validate(listArchiveItemsSchema), asyncHandler(async (req, res) => {
    ...
    if (!isAdmin) {
        where.AND.push({ status: ArchiveStatus.PUBLISHED });
    }
    const [total, items] = await prisma.$transaction([
        ...
        prisma.archiveItem.findMany({
            ...
            skip,
            take: pageSize,
            orderBy: buildOrderBy(typeof req.query.sortBy === 'string' ? req.query.sortBy : undefined, req.query.sortOrder === 'asc' ? 'asc' : 'desc')
        })
    ]);
    res.json(paginated(items.map((item) => ({
        ...item,
...
archiveItemsRouter.get('/:idOrSlug', optionalAuth, validate(archiveItemByIdSchema), asyncHandler(async (req, res) => {
...
archiveItemsRouter.post('/', requireAuth, requireRoles('ADMIN', 'STAFF'), requireCsrf, validate(createArchiveItemSchema), asyncHandler(async (req, res) => {
    ...
    const item = await prisma.archiveItem.create({
        data: {
            ...
            tags: {
                createMany: {
                    data: req.body.tags.map((tagId) => ({ tagId })),
                    skipDuplicates: true
                }
            }
        },
...
archiveItemsRouter.patch('/:id', requireAuth, requireRoles('ADMIN', 'STAFF'), requireCsrf, validate(updateArchiveItemSchema), asyncHandler(async (req, res) => {
        ...
        if (tags) {
            ...
            if (tags.length > 0) {
                await tx.archiveItemTag.createMany({
                    data: tags.map((tagId) => ({ archiveItemId: req.params.id, tagId })),
                    skipDuplicates: true
                });
            }
        }
...
archiveItemsRouter.delete('/:id', requireAuth, requireRoles('ADMIN', 'STAFF'), requireCsrf, validate(archiveItemIdSchema), asyncHandler(async (req, res) => {
    await ensureCanManageItem(req.params.id, req.user);
...
archiveItemsRouter.post('/bulk', requireAuth, requireRoles('ADMIN'), requireCsrf, validate(bulkArchiveActionSchema), asyncHandler(async (req, res) => {
