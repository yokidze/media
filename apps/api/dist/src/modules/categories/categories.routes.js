...
categoriesRouter.get('/', validate(listCategoriesSchema), asyncHandler(async (req, res) => {
...
categoriesRouter.post('/', requireAuth, requireRoles('ADMIN'), requireCsrf, validate(createCategorySchema), asyncHandler(async (req, res) => {
...
categoriesRouter.patch('/:id', requireAuth, requireRoles('ADMIN'), requireCsrf, validate(updateCategorySchema), asyncHandler(async (req, res) => {
    ...
    if (nextName || nextSlug) {
        const duplicate = await prisma.category.findFirst({
            where: {
                ...
                OR: [
                    ...(nextName ? [{ name: { equals: nextName, mode: 'insensitive' } }] : []),
...
categoriesRouter.delete('/:id', requireAuth, requireRoles('ADMIN'), requireCsrf, validate(categoryIdSchema), asyncHandler(async (req, res) => {
