...
filtersRouter.get('/options', asyncHandler(async (_req, res) => {
    ...
    const payload = {
        years: years.map((row) => row.archiveYear).filter((year) => year !== null),
        categories,
        authors,
        tags,
        languages: languages.map((row) => row.language),
        formats: formats.map((row) => row.extension),
        letters: letters.map((row) => row.alphabetLetter).filter((letter) => typeof letter === 'string'),
