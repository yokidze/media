...
const canReadAccessLevel = (roles, accessLevel) => {
...
const ensureCanManageItemFiles = async (itemId, user) => {
...
archiveItemFilesRouter.post('/:itemId/files', requireAuth, requireRoles('ADMIN', 'STAFF'), requireCsrf, upload.array('files', 20), validate(uploadFilesSchema), asyncHandler(async (req, res) => {
...
filesRouter.get('/:id/download', optionalAuth, validate(fileIdSchema), asyncHandler(async (req, res) => {
...
filesRouter.get('/:id/view', optionalAuth, validate(fileIdSchema), asyncHandler(async (req, res) => {
    const file = await prisma.archiveFile.findUnique({
        where: { id: req.params.id },
        include: { archiveItem: true }
    });
...
filesRouter.get('/:id/preview', optionalAuth, validate(fileIdSchema), asyncHandler(async (req, res) => {
    const file = await prisma.archiveFile.findUnique({
        where: { id: req.params.id },
        include: { archiveItem: true }
    });
...
filesRouter.delete('/:id', requireAuth, requireRoles('ADMIN', 'STAFF'), requireCsrf, validate(fileIdSchema), asyncHandler(async (req, res) => {
