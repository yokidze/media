...
const extractManagedStoragePath = (url) => {
    if (!url)
        return null;
    const normalizeManagedPath = (candidate) => {
        try {
...
const toManagedStoragePointer = (relativePath) => `${MANAGED_STORAGE_PREFIX}${relativePath}`;
authRouter.post('/login', authRateLimiter, validate(loginSchema), asyncHandler(async (req, res) => {
...
authRouter.post('/refresh', authRateLimiter, validate(refreshSchema), asyncHandler(async (req, res) => {
...
authRouter.post('/logout', requireAuth, requireCsrf, asyncHandler(async (req, res) => {
...
authRouter.get('/me/avatar', requireAuth, asyncHandler(async (req, res) => {
    const user = await prisma.user.findUnique({
        where: { id: req.user.id },
        select: {
            profilePhotoUrl: true,
...
authRouter.patch('/me', requireAuth, requireCsrf, validate(updateMeSchema), asyncHandler(async (req, res) => {
...
authRouter.post('/change-password', requireAuth, requireCsrf, validate(changePasswordSchema), asyncHandler(async (req, res) => {
    const user = await prisma.user.findUnique({
        where: { id: req.user.id },
...
authRouter.post('/me/avatar', requireAuth, requireCsrf, avatarUpload.single('avatar'), asyncHandler(async (req, res) => {
    const file = req.file;
    if (!file) {
...
authRouter.delete('/me/avatar', requireAuth, requireCsrf, asyncHandler(async (req, res) => {
    const currentUser = await prisma.user.findUnique({ where: { id: req.user.id }, select: meSelect });
    if (!currentUser) {
        res.status(404).json({ error: 'User not found' });
        return;
...
authRouter.get('/me/materials', requireAuth, asyncHandler(async (req, res) => {
