...
const mapPublicRoleToInternal = (role: 'admin' | 'teacher'): RoleName => (role === 'admin' ? RoleName.ADMIN : RoleName.STAFF);
const mapInternalRolesToPublic = (roles: RoleName[]): 'admin' | 'teacher' => (roles.includes(RoleName.ADMIN) ? 'admin' : 'teacher');
...
const inferRoleFromSearch = (tokens: string[]): 'admin' | 'teacher' | undefined => {
  ...
  if (hasAdmin && !hasTeacher) return 'admin';
  if (!hasAdmin && hasTeacher) return 'teacher';
  return undefined;
};

const inferStatusFromSearch = (tokens: string[]): 'active' | 'blocked' | undefined => {
  ...
  if (hasActive && !hasBlocked) return 'active';
  if (!hasActive && hasBlocked) return 'blocked';
  return undefined;
};
...
usersRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const roleWhere =
      requestedRole === 'admin'
        ? { userRoles: { some: { role: { name: 'ADMIN' as const } } } }
        : requestedRole === 'teacher'
          ? { userRoles: { none: { role: { name: 'ADMIN' as const } } } }
          : {};
            ...
            AND: [
              {
                OR: [
                  ...
                  { profile: { is: { fullName: { contains: search, mode: 'insensitive' as const } } } }
                ]
              },
              ...searchTokens.map((token) => ({
                OR: [
                  ...
                  { profile: { is: { fullName: { contains: token, mode: 'insensitive' as const } } } }
                ]
              }))
            ]
          }
